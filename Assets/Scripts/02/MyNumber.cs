...
public class MyNumber : MonoBehaviour {
    [System.Serializable]
    public struct NumToColor {
        public int num;
        public Color color;
    }
    public NumToColor[] colors;
    private Dictionary<int, Color> numToColor;
    ...
    private void InitColor() {
        if (colors != null) {
            numToColor = new Dictionary<int, Color>();
            foreach (var ele in colors) {
                if (!numToColor.ContainsKey(ele.num)) {
                    numToColor.Add(ele.num, ele.color);
                }
            }
        }
    }
    ...
    public void SetValue(int value) {
        ...
        if (numToColor.TryGetValue(value, out Color c)) {
            this.bg.color = c;
        }
    }
